...
void aplicarModo(const ComandoIME &c) {
  ...
  switch (c.modo) {
    ...
    case ModoIME::TORMENTA: fill_solid(leds, NUM_LED, CRGB(255,100,0)); break;
    default: fill_rainbow(leds, NUM_LED, millis()/20, 4); break;
  }
  FastLED.show();
}
...
void loop() {
  ...
  if (millis() - ultimaRecepcion > 5000) {
    // Modo seguro si se pierde el enlace.
    fadeToBlackBy(leds, NUM_LED, 8);
    leds[(millis()/100)%NUM_LED] = CRGB(255,90,0);
    FastLED.show();
  }
  delay(10);
}
